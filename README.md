...
### 1.21.0 — 2026-06-24
- **Added**: Cache Compatibility Mode (#158) — a Settings → Banner Control toggle that keeps the page fully cacheable by LiteSpeed/QUIC.cloud/Varnish/Nginx/WP Rocket. When on, the plugin stops emitting `no-cache`/`no-store` headers and the `DONOTCACHEPAGE` constant for anonymous visitors and renders a single visitor-invariant HTML (default banner, every non-necessary script blocked server-side, no per-country or per-consent variance), so the cached HTML stays safe and the banner runs entirely client-side from the consent cookie. Off by default; keep it off when the banner output varies by country (IAB TCF, geo-targeting, country-targeted banners, runtime geo-routing). Invariance is applied across the initial render, the AMP consent path and the REST banner endpoint, and the short-circuit still honours the `faz_country_dependent_banner_output` filter.
- **Fixed**: the bundled default labels "Always Active", "Show more" and "Show less" are now translatable while preserving admin custom text.
