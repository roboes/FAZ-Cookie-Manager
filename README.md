...
- **Compliance (CCPA/CPRA)**: `[faz_do_not_sell]` relabelled "Do Not Sell **or Share** My Personal Information"; the `sell_personal_data` default is consistent across schema/object layers and documented as covering both sale and sharing.
- **Compliance (Paid Memberships Pro)**: the pay-or-accept auto-grant is now a revocable default — a member's own preference-center choice survives subsequent page loads instead of being re-granted.
