...
- **FlyingPress JS delay/defer/minify exclusion for the consent scripts — issue #125.** FlyingPress ignores the `data-cfasync` / `data-no-optimize` / `data-no-minify` attributes the plugin already prints on its own script tags, so "Delay all JavaScript" would hold the consent banner back until the first user interaction — a late/absent banner defeats pre-consent blocking. The FlyingPress adapter now registers the consent scripts on FlyingPress's `flying_press_exclude_from_delay:js` / `:defer:js` / `:minify:js` filters (added in FlyingPress 4.16 / 5.0), matched by their enqueue-handle ids so the exclusion is independent of the install folder name. No manual keyword exclusion is required. The filter names and additive-merge pattern mirror the FlyingPress integration in Real Cookie Banner (devowl-wp, GPLv3), reimplemented here.
