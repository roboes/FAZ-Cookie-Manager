...
* Compliance (CCPA/CPRA): the `[faz_do_not_sell]` opt-out is relabelled "Do Not Sell or Share My Personal Information" (CPRA covers "sharing" for cross-context behavioural advertising); the per-category `sell_personal_data` default is now consistent across the schema and object layers and documented as covering both sale and sharing.
* Compliance (Paid Memberships Pro): the "pay-or-accept" auto-grant is now a revocable default — a member who opens the preference center and changes or withdraws a category keeps that choice across page loads instead of having it silently re-granted on the next request.
